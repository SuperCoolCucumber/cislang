...
#.drop(['ru_link', 'en_link', 'ka_name', 'ka_link'], axis=1).dropna()
...
def filter1(df):
    df = (df
        .drop(['ru_link', 'en_link', 'ka_name', 'ka_link'], axis=1)
        .dropna()
        .apply({'ru_name': rus_name_align, 'en_name': lambda x: x})
        .apply({'ru_name': delete_patronymic, 'en_name': delete_patronymic})
        )
    return df.drop(df.loc[df["ru_name"] == "Категория:Родившиеся в Тбилиси"].index)


def filter2(df):
    return (df
        .mask(df[['en_name', 'ru_name']].apply(lambda item: item.str.contains(r'\(|\)| X| I | II| V | VI |\,|\.| of '), axis=1)).dropna()
        .apply({'ru_name': delete_inconsistencies, 'en_name': delete_inconsistencies}).dropna()
        .apply({'ru_name': delete_long, 'en_name': delete_long}).dropna()
        .apply(lambda item: item.str.split(" "))
        ...
        .drop_duplicates()
        .to_csv('./cislang/georgian_tst.csv', sep=';', index=False)
        )
